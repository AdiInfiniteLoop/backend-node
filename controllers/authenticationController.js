...
const signup = catchAsync(async (req, res, next) => {
  ...
  const cookieOptions = {
    maxAge: new Date(
      Date.now() + parseInt(process.env.EXPIRES, 10) * 24 * 60 * 60 * 1000
    ),
...
const protect = catchAsync(async (req, res, next) => {
  ...
  if (user.changedPasswordAfter(verified.iat)) {
    return next(
      new ErrorClass('User Recently Changed password! Please Log In Again', 401)
    );
  }
...
const updatePassword = catchAsync(async (req, res, next) => {
  ...
  if (newpassword === password) {
    return next(
      new ErrorClass('Kindly Choose a Different Password from before.', 400)
    );
  }
